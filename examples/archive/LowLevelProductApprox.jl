...
pGM, = prodAppxMSGibbsS(
    ...
    Niter = 1,
    addop = (+, AMP.addtheta),
    diffop = (-, AMP.difftheta),
    getMu = (KDE.getEuclidMu, AMP.getCircMu),
);
#
...
pl = Gadfly.plot(;
    y = pGM[1, :],
    x = pGM[2, :],
    Geom.histogram2d(; xbincount = 50, ybincount = 30),
)
...
pts1 = rand(
    MvNormal([-50; 100.0; 2.5], R_ * [20.0 0 0; 0.0 40.0 0.0; 0.0 0.0 0.05] * (R_')),
    100,
)
...
pGM, = prodAppxMSGibbsS(
    ...
    addop = (+, +, AMP.addtheta),
    diffop = (-, -, AMP.difftheta),
    getMu = (KDE.getEuclidMu, KDE.getEuclidMu, AMP.getCircMu),
);
#
