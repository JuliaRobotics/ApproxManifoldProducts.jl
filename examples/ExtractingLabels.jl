...
function buildDiGraphKernelProduct!(
    ...
    if !haskey(_bd, s)
        ...
        for i = 1:length(dpel.outElements)
            # one vertex for each kernel in the vertex belief
            add_vertex!(dg)
            kers = Symbol(s, "_$i")
            _bd[kers] = nv(dg)
...
function findPathsOfKernels(abg::_AMPDiGraph, dep::Symbol, ind::Symbol)
    ...
    for i = 1:Ni, j = 1:Nj
        ...
        pth1 =
            enumerate_paths(spth, abg.bd[Symbol(ind, "_$j")]) .|> x -> getindex(abg.bd, x)
        strp = string.(pth1)
...
function _getKernelIdInPath(unqpath::AbstractVector{Symbol}, idx::Int)
    ...
    return parse(Int, split(m.match, '_')[end])
end
