...
function testEigenCoords(
  ...
  # spot check
  _ax_ERR = log_lie(SpecialOrthogonal(2), (r_R_ax_')*r_R_ax)[1,2]
  @assert isapprox(_ax_ERR; atol = 5/length(ax_CC)) "Spot check failed on eigen split of manifold points, the estimated point rotation matrix did not match construction."
