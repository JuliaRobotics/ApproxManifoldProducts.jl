...
function _setLabelCombinations!(mdp::DensityProductElements, lbsChs::Dict)
    #
    npts_ = length(lbsChs)
...
function _updateProposalElement(
    mdp::DensityProductElements,
    elemval,
    densid::Int,
    elemidx::Int,
...
function _sequenceUpdateProposalElement(
    mdp::DensityProductElements,
    elemval,
    densid::Int,
    elemidx::Int,
...
function _listOutElementLabelSelections(mdp::DensityProductElements, idx::Int)
    return mdp.lblCombinations[idx]
end

function _findOutElementConnectedLabel(mdp::DensityProductElements, densidx, idx::Int)
    outelems = Int[]
    # check for all elements
    for outi = 1:length(mdp.outElements)
