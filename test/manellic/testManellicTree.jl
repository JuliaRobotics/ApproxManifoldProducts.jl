...
## ========================================================================================
@testset "Test Product the brute force way, SpecialEuclideanGroup(2; variant = :right)" begin
    M = SpecialEuclideanGroup(2; variant = :right)
    ε = identity_element(M)
    ...
    function globalCov(M, ker)
        p = mean(ker)
        pΣp = cov(ker)
        # ? adjoint_action(M, p, pΣp)
    ...
    lines!(xs, normalize(amp_gl_pqs[:, 60, 30]))

    contour(xs, ys, pdf_pqs[:, :, 30]; color = :blue)
    contour!(xs, ys, amp_gl_pqs[:, :, 30]; color = :green)

    #FIXME -- brute force will be more accurate than approx product, relax these tests for stochastic variability
    normalized_compare_test =
        isapprox.(normalize(amp_pqs), normalize(amp_bf_pqs); atol = 0.001)
...
@testset "Manellic tree bandwidth optimization 1D section search" begin
    ...
    res = Optim.optimize((s) -> cost(pts, s^2), 0.05, 3.0, Optim.GoldenSection())
    ...
    res = Optim.optimize((s) -> cost2(s^2), 0.05, 3.0, Optim.GoldenSection())
    ...
    # and optimize with "update" kernel bandwith cost
    res = Optim.optimize((s) -> cost3(s^2), 0.05, 3.0, Optim.GoldenSection())
